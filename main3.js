...
function canales(){
  ...
  var res = { 
              "emitiendo": [],
              "offline": []
            }; //va a ser un objeto tipo diccionario con 2 claves: emitiendo y offline, y sus respectivos valores va a ser un array de diccionarios con el par nombre: status o también podría ser [status, logo, url]
  var i;
  for (i= 0; i < users.length; i++){
    //chequear si transmite o no
    if (info[i].stream != null){
      //transmite
      var clave = info[i].stream.name;
      var valor = info[i].stream.status;
      var direc = info[i]._links.channel;
      res["emitiendo"].push({ nombre: clave, status : valor, url : direc});
     } else{
      //no transmite
      var clave = info[i].display_name;
      var direc = info[i]._links.channel;
      res["offline"].push({ nombre: clave, status : "Offline", url : direc});
    }
  }
  return res;
}
...
function emitiendo(){
  //recorre canales["emitiendo"] 
  //para cada uno mostrás la clave y el valor
  //darle la forma de lista de definición
}

function offline(){
  //recorre canales["offline"]
}
